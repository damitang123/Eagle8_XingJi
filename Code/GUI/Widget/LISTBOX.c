...
int LISTBOX_OwnerDraw(const WIDGET_ITEM_DRAW_INFO* pDrawItemInfo) {
  switch (pDrawItemInfo->Cmd) {
    ...
    case WIDGET_ITEM_DRAW: {
      ...
      rText.x0 = pDrawItemInfo->x0 + 5;		//modify by elvin
	...
	/*添加项目分隔横线,add by elvin*/
	GUI_SetColor(GUI_LIGHTGRAY);
	GUI_DrawHLine(pDrawItemInfo->y0 + pItem->ySize - 1,pDrawItemInfo->x0,r.x1);
...
*       _OnTouch
*/
#define LISTBOX_SUPPORT_SCROLL		1
static void _OnTouch(LISTBOX_Handle hObj, WM_MESSAGE* pMsg) {
  const GUI_PID_STATE* pState = (const GUI_PID_STATE*)pMsg->Data.p;
#if LISTBOX_SUPPORT_SCROLL
  static int yLast = -1;
  static unsigned char bScrollEvent = 0;
  if (pMsg->Data.p) {  /* Something happened in our area (pressed or released) */
	...
	{
	    	WM_HWIN hWin=WM_GetDialogItem(hObj, GUI_ID_VSCROLL);
		LISTBOX_Obj* pObj = LISTBOX_H2P(hObj);
		unsigned short nOffset = (pObj->ItemSpacing ) >> 1;
		if(pState->y>=yLast)
		{
			if((pState->y-yLast) > nOffset)
			{
				yLast = pState->y + nOffset;
				bScrollEvent = 1;
				SCROLLBAR_Dec(hWin);
			}
		}
