...
*       _OnTouch
*/
#define MUTIEDIT_SUPPORT_SCROLL		1
static void _OnTouch(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj, WM_MESSAGE*pMsg) {
  int Notification;
  const GUI_PID_STATE* pState = (const GUI_PID_STATE*)pMsg->Data.p;
  #if MUTIEDIT_SUPPORT_SCROLL
  static int yLast = -1;
  static char bAreadyUp;
  static char bAreadyDown;
   if (pMsg->Data.p) {  /* Something happened in our area (pressed or released) */
	...
	{
		unsigned short nOffset = 30;
		if(pState->y>=yLast)
		{
			if((pState->y-yLast) > nOffset && bAreadyUp == 0)
			{
				bAreadyUp = 1;
				bAreadyDown = 0;
				yLast = pState->y + nOffset;
				_SetCursorXY(hObj, pObj, 0, 0);
				_Invalidate(hObj);
			}
		}
		else
		{
			if((yLast-pState->y)>nOffset && bAreadyDown == 0)
			{
				bAreadyUp = 0;
				bAreadyDown = 1;
				yLast = pState->y - nOffset;
				_SetCursorXY(hObj, pObj, 0, 0x7FFF);
				_Invalidate(hObj);
			}
		}
	}
