...
/* Define colors */
  ...
  #define BUTTON_BKCOLOR0_DEFAULT   0xE3D9CD	//0xAAAAAA
#endif
...
/* Draw the 3D effect (if configured) */
   if(pObj->Btn_CF&BUTTON_CF_3D)	//modify by elvin
  {
    int EffectSize;
    if ((PressedState) == 0) {
...
//   LCD_SetColor(0x000000);
  ...
  if(pObj->Btn_CF&BUTTON_CF_CIRCLE_ANGLE) //modify by elvin
 {
 	/*自定义绘图和填充方法,前提是按钮必须加通明属性*/
	#define X 8
	GUI_RECT r=rInside;
	GUI_COLOR  Color=pObj->Props.aBkColor[ColorIndex];
	LCD_SetColor(Color);
	GUI_FillCircle(X,X,X);	/*左上角的圆*/
	LCD_SetColor(0x5E6F72);
	GUI_DrawCircle(X,X,X);
	
	LCD_SetColor(Color);
	GUI_FillCircle(X,r.y1-X,X);/*左下角的圆*/
	LCD_SetColor(0x5E6F72);
	GUI_DrawCircle(X,r.y1-X,X);
	
	LCD_SetColor(Color);
	GUI_FillCircle(r.x1-X,X,X);/*右上角的圆*/
	LCD_SetColor(0x5E6F72);
	GUI_DrawCircle(r.x1-X,X,X);
	
	LCD_SetColor(Color);
	GUI_FillCircle(r.x1-X,r.y1-X,X);/*右下角的圆*/
	LCD_SetColor(0x5E6F72);
	GUI_DrawCircle(r.x1-X,r.y1-X,X);
	
	LCD_SetColor(Color);
	GUI_FillRect(X,0,r.x1-X,r.y1);
	GUI_FillRect(0,X,X,r.y1-X);
	GUI_FillRect(r.x1-X,X,r.x1,r.y1-X);

	LCD_SetColor(0x5E6F72);
	GUI_DrawHLine(0, X, r.x1-X);
	GUI_DrawHLine(r.y1, X, r.x1-X);
	GUI_DrawVLine(0, X, r.y1-X);
	GUI_DrawVLine(r.x1, X, r.y1-X);
...
BUTTON_Handle BUTTON_CreateEx(int x0, int y0, int xsize, int ysize, WM_HWIN hParent, int WinFlags, int ExFlags, int Id) {
  ...
  if (hObj) {
    ...
    pObj->Btn_CF=ExFlags;	//add by elvin
  } else {
